...
Best-K resolution (cardinality only):
  default → read cardinality_qaoa_solution.best_K from the latest
  ring_xy_batch_*.json. --rederive-k or no results JSON found → run
  solve_with_qaoa_cardinality on every K to find the best.
"""
...
def prepare_problem_data(experiment):
...
def run_convergence(args, problems, results):
    ...
    for pid, experiment in enumerate(problems):
        ...
        # Build the HOPO object whenever it is needed (integer-HUBO rows, the
        # exact spectrum for the approximation ratio, or the per-qubit bucket).
        portfolio_hubo = None
        if want_hopo:
            portfolio_hubo = build_portfolio_hubo(experiment, data)
...
def main():
    ...
    results_path = args.results_json or discover_results_json(args.method)
    results = load_results(results_path)
    # best_K (cardinality only) is the sole consumer of the results JSON.
