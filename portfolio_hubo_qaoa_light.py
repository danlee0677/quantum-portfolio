...
from utils import (
    ...
    bitstrings_to_optimized_portfolios,
    int_to_bitstring,
...
class HigherOrderPortfolioQAOA:
    def __init__(
        ...
        self.n_qubits = 0
        for asset in stocks:
            self.assets_to_qubits[asset] = list(
                range(self.n_qubits, self.n_qubits + self.num_qubits_per_asset[asset])
            )
        ...
        if strict_budget_constraint:
            self.budget_constraint = self.construct_budget_constraint_strict(
                scaler=lambda_budget
            )
        else:
            self.budget_constraint = self.construct_budget_constraint(
                scaler=lambda_budget
            )

        print("Total number of qubits: ", self.n_qubits)
        assert max_qubits >= self.n_qubits, (
            "Number of qubits exceeds the maximum number of qubits"
        )
    ...
    def construct_cost_hubo_int(self):
            ...
            for i in range(self.num_assets):
                for j in range(self.num_assets):
                    for k in range(self.num_assets):
                        skew = (self.risk_aversion / 6) * self.coskewness_tensor[i][j][
                            k
                        ]
                        self.cost_hubo_int[
                            (self.stocks[i], self.stocks[j], self.stocks[k])
                        ] = -skew
        ...
        if self.cokurtosis_tensor is not None:
            for i in range(self.num_assets):
                for j in range(self.num_assets):
                    for k in range(self.num_assets):
                        for l in range(self.num_assets):  # noqa
                            kurt = (self.risk_aversion / 24) * self.cokurtosis_tensor[
                                i
                            ][j][k][l]
                            self.cost_hubo_int[
                                (
    ...
    def construct_budget_constraint(self, scaler=1):
        ...
        for asset1, asset2 in itertools.combinations(self.stocks, 2):
            budget_const[(asset1, asset2)] = (
                2 * scaler * self.prices_now[asset1] * self.prices_now[asset2]
            )
        return budget_const
    ...
    def construct_budget_constraint_strict(self, scaler=1):
        ...
        self.budget_dicretization_unit = gcd.reduce(
            asset_prices_now + [int(self.budget)]
        )
        print("Budget discretization unit: ", self.budget_dicretization_unit)
        qubits_for_budget = int(
            np.floor(np.log2(self.budget / self.budget_dicretization_unit))
        )
        last_budget_qubit = self.n_qubits + qubits_for_budget
        budget_for_last_qubit = (
            self.budget - self.budget_dicretization_unit * 2**qubits_for_budget
        )
        slacks = []
        # for i in range(qubits_for_budget):
        self.assets_to_qubits["slack_int"] = list(
            range(self.n_qubits, self.n_qubits + qubits_for_budget)
        )
        slacks.append("slack_int")
        self.n_qubits += qubits_for_budget
        self.assets_to_qubits["slack_int_last"] = [last_budget_qubit]
        slacks.append("slack_int_last")
        self.n_qubits += 1

        budget_qubit_coeffs = {
            "slack_int": self.budget_dicretization_unit,
            "slack_int_last": budget_for_last_qubit,
        }
        ...
        for v1, v2 in itertools.combinations(all_vars, 2):
            if v1 in self.stocks and v2 in self.stocks:
                budget_const[(v1, v2)] = (
                    2 * scaler * self.prices_now[v1] * self.prices_now[v2]
                )
            elif v1 in slacks and v2 in slacks:
                budget_const[(v1, v2)] = (
                    2 * scaler * budget_qubit_coeffs[v1] * budget_qubit_coeffs[v2]
                )
            elif v1 in self.stocks and v2 in slacks:
                budget_const[(v1, v2)] = (
                    -2 * scaler * self.prices_now[v1] * budget_qubit_coeffs[v2]
                )
            elif v1 in slacks and v2 in self.stocks:
                budget_const[(v1, v2)] = (
                    -2 * scaler * self.prices_now[v2] * budget_qubit_coeffs[v1]
                )
            ...
            if len(integer_var) == 1:
                ...
                for n, qubit in enumerate(self.assets_to_qubits[asset]):
                    ...
                    if self.log_encoding:
                        if bin_var in self.cost_hubo_bin:
                            self.cost_hubo_bin[bin_var] += (2**n) * self.cost_hubo_int[
                                integer_var
                            ]
                        else:
                            self.cost_hubo_bin[bin_var] = (2**n) * self.cost_hubo_int[
                ...
                for n, qubit0 in enumerate(self.assets_to_qubits[asset0]):
                    for m, qubit1 in enumerate(self.assets_to_qubits[asset1]):
                        ...
                        if self.log_encoding:
                            if bin_var in self.cost_hubo_bin:
                                self.cost_hubo_bin[bin_var] += (
                                    2 ** (n + m) * self.cost_hubo_int[integer_var]
                                )
                            else:
                                self.cost_hubo_bin[bin_var] = (
                                    2 ** (n + m) * self.cost_hubo_int[integer_var]
                            ...
                            self.cost_hubo_bin[bin_var] = self.cost_hubo_int[
                                integer_var
                            ]
                ...
                for n, qubit0 in enumerate(self.assets_to_qubits[asset0]):
                    for m, qubit1 in enumerate(self.assets_to_qubits[asset1]):
                        for k, qubit2 in enumerate(self.assets_to_qubits[asset2]):
                            ...
                            if self.log_encoding:
                                self.cost_hubo_bin[bin_var] = (
                                    2 ** (n + m + k) * self.cost_hubo_int[integer_var]
                                )
                            else:
                                self.cost_hubo_bin[bin_var] = self.cost_hubo_int[
                                    integer_var
                        ...
                        for k, qubit2 in enumerate(self.assets_to_qubits[asset2]):
                            for l, qubit3 in enumerate(self.assets_to_qubits[asset3]):
                                ...
                                if self.log_encoding:
                                    self.cost_hubo_bin[bin_var] = (
                                        2 ** (n + m + k + l)
                                        * self.cost_hubo_int[integer_var]
                                    )
                                else:
                                    self.cost_hubo_bin[bin_var] = self.cost_hubo_int[
                                        integer_var
    ...
    def simplify_cost_hubo_bin(self):
        ...
        for bin_var in self.cost_hubo_bin:
            ...
            if bin_var_set in self.cost_hubo_bin_simplified:
                self.cost_hubo_bin_simplified[bin_var_set] += self.cost_hubo_bin[
                    bin_var
                ]
            else:
    ...
    def cost_hubo_bin_to_ising_hamiltonian(self):
        ...
        for bin_var_set in self.cost_hubo_bin_simplified:
            ...
            if len(bin_var) == 1:
                qubit0 = bin_var[0][1]
                self.hamiltonian += (coeff / 2) * (
                    qml.Identity(qubit0) - qml.PauliZ(qubit0)
                )
                ...
                self.hamiltonian += (
                    (coeff / 4)
                    * (qml.Identity(qubit0) - qml.PauliZ(qubit0))
                    @ (qml.Identity(qubit1) - qml.PauliZ(qubit1))
                )
                ...
                self.hamiltonian += (
                    ...
                    * (qml.Identity(qubit0) - qml.PauliZ(qubit0))
                    @ (qml.Identity(qubit1) - qml.PauliZ(qubit1))
                    @ (qml.Identity(qubit2) - qml.PauliZ(qubit2))
                )
                ...
                self.hamiltonian += (
                    ...
                    * (qml.Identity(qubit0) - qml.PauliZ(qubit0))
                    @ (qml.Identity(qubit1) - qml.PauliZ(qubit1))
                    @ (qml.Identity(qubit2) - qml.PauliZ(qubit2))
                    @ (qml.Identity(qubit3) - qml.PauliZ(qubit3))
                )
    ...
    def get_QAOA_circuits(self):
        ...
        mixer_hamiltonian = qml.qaoa.x_mixer(
            range(self.n_qubits)
        )  # qml.qaoa.xy_mixer(complete_graph)
    ...
    def get_compiled_QAOA_circuits(self):
        ...
        qaoa_probs_circuit = qml.compile(
    ...
    def draw_qaoa_circuit(self):
        fig, ax = qml.draw_mpl(
            self.qaoa_circuit, expansion_strategy="device", decimals=2
        )(self.init_params)
    ...
    def solve_with_continuous_variables(self):
        if self.coskewness_tensor is None and self.cokurtosis_tensor is None:
            ...
            allocation, left_overs = hef.get_discrete_allocation(
                weights, self.prices_now, self.budget
            )
            print("Left over budget: ", left_overs)
    ...
    def solve_with_continuous_variables_unconstrained(self):
        if self.coskewness_tensor is None and self.cokurtosis_tensor is None:
            raise ValueError(
                "Unconstrained optimization is only possible with higher moments"
            )
        else:
            hef = HigherMomentPortfolioOptimizer(
                self.stocks,
            ...
            print(
                "Optimized Weights with unconstrained classical continuous variable (considering variance, skewness and kurtosis):"
            )
            for asset, weight in weights.items():
                print(f"{asset}: {weight:.2%}")
            ...
            value = self.get_objective_value(allocation)
            print(
                "Maximized utility from continuous higher moments for unconstrained: ",
                value,
            )
    ...
    def solve_exactly(self):
        if self.n_qubits < 14:
            ...
            self.smallest_eigenvalues, self.smallest_eigenvectors, eigenvalues = (
        ...
        optimized_portfolio = bitstrings_to_optimized_portfolios(
            self.smallest_bitstrings, self.assets_to_qubits
        )
        result1 = self.satisfy_budget_constraint(optimized_portfolio)
        eigenvalues = [float(v) for v in eigenvalues]
        objective_values = [
            float(self.get_objective_value(allocation))
            for allocation in optimized_portfolio
        ]
        ...
        if first_excited_state is not None:
            second_smallest_bitstrings = [
                basis_vector_to_bitstring(first_excited_state)
            ]
            second_optimized_portfolio = bitstrings_to_optimized_portfolios(
                second_smallest_bitstrings, self.assets_to_qubits
            )
            result2 = self.satisfy_budget_constraint(second_optimized_portfolio)
            objective_values = [
                float(self.get_objective_value(allocation))
                for allocation in second_optimized_portfolio
            ]
    ...
    def satisfy_budget_constraint(self, optimized_portfolio):
        ...
        for portfolio in optimized_portfolio:
            ...
            results.append(
                {"portfolio": portfolio, "budget": B, "difference": self.budget - B}
            )
        return results
    ...
    def solve_with_qaoa(self):
        ...
        two_most_probable_states = [
            int_to_bitstring(i, self.n_qubits) for i in two_most_probable_states
        ]
        optimized_portfolios = bitstrings_to_optimized_portfolios(
            two_most_probable_states, self.assets_to_qubits
        )
    ...
    def build_integer_hubo_circuit(self, device_name="lightning.qubit", layers=None):
        """
        Construct the integer-HUBO QAOA circuit without running optimization.
        Returns (qaoa_circuit, qaoa_probs_circuit, layers_used, n_qubits).
        """
        layers_used = self.layers if layers is None else layers
        ...
        def qaoa_circuit(params):
            ...
            alphas = params[layers_used:]
            for wire in range(self.n_qubits):
                qml.Hadamard(wires=wire)
            qml.layer(qaoa_layer, layers_used, gammas, alphas)
            return qml.expval(cost_hamiltonian)
        ...
        return qaoa_circuit, qaoa_probs_circuit, layers_used, self.n_qubits
    ...
    def solve_with_qaoa_cma_es(self):
        ...
        qaoa_circuit, qaoa_probs_circuit, layers_used, _ = (
            self.build_integer_hubo_circuit()
        )
        ...
        es = cma.CMAEvolutionStrategy(
            initial_params, sigma0=0.1, options={"maxiter": maxiter}
        )
        ...
        two_most_probable_states = [
            int_to_bitstring(i, self.n_qubits) for i in two_most_probable
        ]
        optimized_portfolios = bitstrings_to_optimized_portfolios(
            two_most_probable_states, self.assets_to_qubits
        )
        ...
        objective_values = [
            float(self.get_objective_value(optimized_portfolios[i])) for i in range(2)
        ]
    ...
    def get_objective_value(self, optimized_portfolio):
        ...
        for i in range(len(self.stocks)):
            objective_value -= (
                optimized_portfolio[self.stocks[i]] * self.expected_returns[i]
            )
        ...
        for i in range(len(self.stocks)):
            for j in range(len(self.stocks)):
                objective_value += (
                    ...
                    * optimized_portfolio[self.stocks[i]]
                    * optimized_portfolio[self.stocks[j]]
            ...
            for i in range(len(self.stocks)):
                for j in range(len(self.stocks)):
                    for k in range(len(self.stocks)):
                        objective_value -= (
                            (self.risk_aversion / 6)
                            * optimized_portfolio[self.stocks[i]]
                            * optimized_portfolio[self.stocks[j]]
                            * optimized_portfolio[self.stocks[k]]
                            * self.coskewness_tensor[i][j][k]
                        )
                ...
                for j in range(len(self.stocks)):
                    for k in range(len(self.stocks)):
                        for l in range(len(self.stocks)):
                            objective_value += (
                                (self.risk_aversion / 24)
                                * optimized_portfolio[self.stocks[i]]
                                * optimized_portfolio[self.stocks[j]]
                                * optimized_portfolio[self.stocks[k]]
                                * optimized_portfolio[self.stocks[l]]
                                * self.cokurtosis_tensor[i][j][k][l]
                            )
